...
def pot_dest(goal):
    if type(goal) is pd.core.frame.DataFrame: 
            w_gdf = gpd.GeoDataFrame(crs=4326,
...
home_csv = filedir+"locationdata/utrecht_homelatlon.csv"
...
#duration and travel mean comes from the getroute funciton. "travel mean" is not used for generating the schedule, only as an input so that the generated schedule has "travel_mean". 
#wo means work and sports, for one person. input travel duration (time in seconds)
def schedule_general_wo (duration, travelmean, filedir, free_duration = 1, name = "work_sport", save_csv = True , time_interval=0.01):
  #print("ready")
  ...
  if work2home_end >20.5: # at home if goes home late already. 
      ...
      # before go to work in the morning, people's free time after abour 1.5 hours
  ...
  start_time = np.round(np.array([0.0, home2work_start, work_start, work2home_start, work2home_end, outdoor_evening, outdoor_evening+free_duration]),2)
...
#  activity_type_code = [1, 2, 1, 2, 1, freetime_type, 1]
  ...
  data = [start_time,end_time,activity, activity_code, [travelmean], [duration] ]              
   
  schedule = pd.DataFrame(data=data).T
  schedule = schedule.rename (columns = {0:"start_time", 1: "end_time", 2:"activity", 3:"activity_code", 4: "travel_mean", 5:"duration"})
  if save_csv:
...
# @f_d: probability distribution of travel mode
#@df file includes start destination coordinates, 
def getroute(id, df, f_d, Gw, Gb, Gd, speed_walk = 5, speed_bike = 14):
    ...
    #traveldistance is calculated using the route of min length. so not exactly the same as travel time.
        ...
        except: #exception raised when there is no path between two points. then we just use the home locaiton. 
            travel_distance = 0 
            travel_time = 0
            route_ = Point(start)
            print("no road")
